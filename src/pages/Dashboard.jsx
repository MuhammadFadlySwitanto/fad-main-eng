...
import MyDocument from '../components/MyDocument';
  ...
  function Dashboard() {
    ...
    const dashboardStaticData = () =>  [
      ...
      { title: 'Boiler Gas Usage', value: `${parseFloat(data.Total_Gas_Boiler).toFixed(3)} MBTU` },
      { title: 'Total Boiler Cost', value: `${(data.Total_Gas_Boiler * dataTotalUangGas).toLocaleString('id-ID', { style: 'currency', currency: 'IDR' })}` },
      { title: 'PDAM Water Usage', value: `${data.PDAM} kubik` },
      { title: 'Total PDAM Cost', value: `${(data.PDAM * dataTotalUangAir).toLocaleString('id-ID', { style: 'currency', currency: 'IDR' })}` },
...
// ==============================================================   SUBMIT   ============================================================================================  
  ...
  useEffect(() => {
    ...
    const totalCostValue = (MVMDP * dataTotalUang) +
        ...
        <div className="min-h-screen bg-cobabg mx-auto p-4 md:p-6 2xl:p-10 relative">
          ...
          <div className="flex flex-col space-y-6 md:space-y-0 md:flex-row-reverse justify-between">
            <div className="flex flex-wrap items-start justify-end gap-x-4 gap-y-2">
              ...
              >
                <HiOutlinePrinter size={24} className="flex-shrink-0 h-6 w-6 text-text -ml-1 mr-2"/>Print PDF
            ...
            <div className="p-8 shadow-buatcard bg-coba rounded-lg dark:border-strokedark cursor-pointer transform transition duration-300 hover:scale-105 active:scale-65">
              <div className="flex items-center mb-1">
                ...
                <div>
                  ...
                  <span className="inline-block text-text text-xl font-semibold truncate">{parseFloat(data.Total_Gas_Boiler).toFixed(3) ?? "Loading..."}</span>
          ...
          <div className="hidden">
            <PDAM ref={pdamRef} />
            <NVMDP ref={nvmdpRef} getLimit={getLimit} dataTotalUang={dataTotalUang} />
          </div>
